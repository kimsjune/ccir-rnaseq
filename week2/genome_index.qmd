...
cd /home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop
...
Our files are `gz`ipped (compressed). So they have to be un-zipped with `gunzip`.  
...
gunzip -c gencode.vM1.annotation.gtf.gz > ./gencode.vM1.annotation.gtf
# Are the files there?
ls -l
```
...
-  What about the `gunzip` thing just now? It's okay for a log in node 
-  We need to write a text file (`job script`) describing our command, then submit it to `slurm`  
...
## Actual command to create an index with `STAR`

**After** the header comes the actual command we want to run.
...
```{bash}
STAR --runThreadN 12 --runMode genomeGenerate --genomeDir /home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/stargenome --genomeFastaFiles /home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/mm10.fa  --sjdbGTFfile /home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/gencode.vM1.annotation.gtf --sjdbOverhang 74
```
...
--genomeDir \
/home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/stargenome \
--genomeFastaFiles \
/home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/mm10.fa \
--sjdbGTFfile \
/home/$USER/projects/$SLURM_ACCOUNT/$USER/workshop/gencode.vM1.annotation.gtf \
--sjdbOverhang 74
```
